...
for first_concept in config["concepts"]:
    for second_concept in config["concepts"]:
        ...
        pos_pos_implication = count_pp / count_p
        pos_neg_implication = 1 - pos_pos_implication
        neg_pos_implication = count_np / count_n
        neg_neg_implication = 1 - neg_pos_implication
        jaccard_distance = 1 - (count_pp / (count_p + count_np))

        rules['pIMPLp'].append([pos_pos_implication, first_concept, second_concept])        
        rules['pIMPLn'].append([pos_neg_implication, first_concept, second_concept])        
        rules['nIMPLp'].append([neg_pos_implication, first_concept, second_concept])        
        rules['nIMPLn'].append([neg_neg_implication, first_concept, second_concept])     
...
for rule_type, rule_instances in rules.items():
    for confidence_threshold in [0.7, 0.8, 0.9, 0.95, 0.99]:
        filtered_list = list(filter(lambda x: x[0][1] >= confidence_threshold, rule_instances))
        if len(filtered_list) == 0:
            ...
            continue
        training_threshold = min(map(lambda x: x[0][0], filtered_list), confidence_threshold)
